from model_network_private_own_class_extreme_transaction_20250211 import TestModelINTEST
...
# add customs graph
customs_network = nx.Graph()
# Read the CSV file and add edges to the graph
with open('incoming_and_outgoing_customs_network.csv', 'r', encoding='utf-8-sig') as file:
    reader = csv.reader(file, delimiter=';')
    for line in reader:
        source, target, weight = map(float, line)
        customs_network.add_edge(source, target, weight=weight)
...
def run_model(run_number, fixed_agent_behaviors=None):
    ...
    starter_model = TestModelINTEST(G, agents_per_node, customs_network, agent_behaviors=fixed_agent_behaviors)
    ...
    step_count = 0
    max_steps = 1500  # Safety mechanism to prevent infinite loop
    ...
    output_filename = f'Runs/agents_network_private_own_class_extreme_transaction_run_{run_number}_20250210.csv'
...
if all_dataframes:
    ...
    id_average_output_file = 'Runs/agents_network_private_own_class_extreme_transaction_id_averages_20250211.csv'
    ...
    class_average_output_file = 'Runs/agents_network_private_own_class_class_extreme_transaction_averages_20250211.csv'
