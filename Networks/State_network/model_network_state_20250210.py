    ...
    def step(self):
        ...
        if neighbors:
            ...
            for node in potential_moves:
                ...
                previous_customs_weight = getattr(self, 'last_traveled_customs_edge_weight', 0)
                customs_cost = len(
                    self.goods_list) * 0  # Customs tax depends on previous movement - in this case gfor agent.state customs=0
                ...
                if total_cost < min_total_cost and self.wealth >= total_cost:
                    min_total_cost = total_cost
                    best_move = node
                    best_customs_cost =  0 # due to the customs for the agent.type being 0
...
class TestModelINTEST(mesa.Model):
    ...
    def __init__(self, G, agents_per_node, customs_network, agent_behaviors=None):
        ...
        for node in G.nodes:
            ...
            agent_state = AgentState(unique_id=self.next_id(), model=self)
            agent_state.goods_list = copy.deepcopy(agent_state.goods_list)
            agent_state.agent_behavior = behavior
            self.grid.place_agent(agent_state, node)
            self.schedule.add(agent_state)
            ...
            while len(agents_on_node) > 1:
                ...
                for i in range(len(selected_agents)):
                    for j in range(i + 1, len(selected_agents)):
                        ...
                        if isinstance(agent1, AgentState) and isinstance(agent2, AgentState):
