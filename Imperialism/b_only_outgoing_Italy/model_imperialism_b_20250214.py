    ...
    def step(self):
        ...
        if neighbors:
            ...
            for node in potential_moves:
                ...
                # Incoming customs cost based on previous movement, using the incoming_customs network
                previous_incoming_edge_weight = self.last_traveled_incoming_customs_edge_weight if hasattr(self,
                                                                                 'last_traveled_edge_weight') else 0
                incoming_customs_cost = len(self.goods_list) * previous_incoming_edge_weight

                # Outgoing customs cost based on previous movement, using the outcoming_customs network
                previous_outgoing_edge_weight = self.last_traveled_outgoing_customs_edge_weight if hasattr(self, 'last_traveled_edge_weight') else 0

                outgoing_customs_cost = 0


                customs_cost = incoming_customs_cost + outgoing_customs_cost
            ...
            if best_move is not None:
                self.last_traveled_incoming_customs_edge_weight = self.model.incoming_customs_network.edges[self.pos, best_move][
                    'weight'] if self.model.incoming_customs_network.has_edge(self.pos, best_move) else 0
                self.last_traveled_outgoing_customs_edge_weight = self.model.outgoing_customs_network.edges[self.pos, best_move][
                    'weight'] if self.model.outgoing_customs_network.has_edge(self.pos, best_move) else 0
    ...
    def step(self):
        ...
        if neighbors:
            ...
            for node in potential_moves:
                ...
                # Outgoing customs cost based on previous movement, using the outcoming_customs network
                previous_outgoing_edge_weight = self.last_traveled_outgoing_customs_edge_weight if hasattr(self,
                                                                                                           'last_traveled_edge_weight') else 0
            ...
            # If a valid move is found, move the agent
            if best_move is not None:
                self.last_traveled_incoming_customs_edge_weight = \
                self.model.incoming_customs_network.edges[self.pos, best_move][
                    'weight'] if self.model.incoming_customs_network.has_edge(self.pos, best_move) else 0
                self.last_traveled_outgoing_customs_edge_weight = \
                self.model.outgoing_customs_network.edges[self.pos, best_move][
                    'weight'] if self.model.outgoing_customs_network.has_edge(self.pos, best_move) else 0
                self.last_traveled_transport_edge_weight = self.model.G.edges[self.pos, best_move][
                    'weight']
                self.model.grid.move_agent(self, best_move)
    ...
    def step(self):
        ...
        if neighbors:
            ...
            for node in potential_moves:
                ...
                previous_incoming_edge_weight = self.last_traveled_incoming_customs_edge_weight if hasattr(self,
                                                                                                           'last_traveled_edge_weight') else 0
                incoming_customs_cost = 0

                # Outgoing customs cost based on previous movement, using the outcoming_customs network
                previous_outgoing_edge_weight = self.last_traveled_outgoing_customs_edge_weight if hasattr(self,
                                                                                                           'last_traveled_edge_weight') else 0

                outgoing_customs_cost = len(self.goods_list) * previous_outgoing_edge_weight
...
class TestModelINTEST(mesa.Model):
    ...
    def __init__(self, G, agents_per_node, incoming_customs_network, outgoing_customs_network, agent_behaviors=None):
        """
        ...
        """
        self.G = G
        self.incoming_customs_network = incoming_customs_network
        self.outgoing_customs_network = outgoing_customs_network
            ...
            while len(agents_on_node) > 1:
                ...
                for i in range(len(selected_agents)):
                    for j in range(i + 1, len(selected_agents)):
                        ...
                        cost = 0 if agent1.type_id == agent2.type_id else 1
