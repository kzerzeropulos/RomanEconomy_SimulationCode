    ...
    def meet_agents(self):
        ...
        for node in self.G.nodes:
            ...
            while len(agents_on_node) > 1:
                # Compute transaction costs for all possible pairs
                pair_costs = []
                for i in range(len(agents_on_node)):
                    for j in range(i + 1, len(agents_on_node)):
                        agent1 = agents_on_node[i]
                        agent2 = agents_on_node[j]
                        # Transaction cost: 0 if same type, otherwise 1
                        #cost = 1 if agent1.type_id == agent2.type_id else 5
                        cost = 5
