from test_model_final_transactioncost_uneventrade_20241219 import TestModelINTEST
...
with open('all_0.csv', 'r', encoding='utf-8-sig') as file:
...
def run_model(run_number, fixed_agent_behaviors=None):
    ...
    step_count = 0
    max_steps = 10000  # Safety mechanism to prevent infinite loop
    ...
    output_filename = f'Runs/agents_goods_transactioncost_lowcostifsametype_run_{run_number}_20250106_3.csv'
    ...
    fieldnames = ['Run', 'Step Count', 'Agent ID', 'Agent Class', 'Total Wealth', 'Trade Behavior'] + list(all_goods)
...
if all_dataframes:
    ...
    average_data_by_id = combined_data.drop(columns=['Run', 'Step Count']).groupby(
        ['Agent ID', 'Agent Class', 'Trade Behavior']).mean()

    # Save the averages by Agent ID to a new CSV file
    id_average_output_file = 'Runs/agents_goods_behavior_transaction_lowcostifsametype_id_averages_20250106_3.csv'
    ...
    average_data_by_class = combined_data.drop(columns=['Run', 'Step Count', 'Agent ID', 'Trade Behavior']).groupby(
        ['Agent Class']).mean()

    # Save the class-based averages to a new CSV file
    class_average_output_file = 'Runs/agents_goods_behavior_lowcostifsametype_class_averages_20250106_3.csv'
