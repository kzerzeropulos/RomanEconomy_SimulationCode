    ...
    def step(self):
        ...
        if neighbors:
            ...
            for node in potential_moves:
                ...
                # Calculate the cost based on the price of all goods the agent possesses
                total_goods_price = sum(good[2] for good in self.goods_list)
                total_cost = edge_weight * total_goods_price

                if total_cost < min_cost and self.wealth >= total_cost:
                    min_cost = total_cost
                    best_move = node
