...
class AgentItaly(mesa.Agent):
    def __init__(self, unique_id, model):
        ...
        self.type = "Italy"
        self.type_id = 6
        self.goods_list = [("ItalianPottery", 1, 10)] * 10
        self.active = True
        self.agent_behavior = None
        self.trades_per_step = 0
        self.has_traded = False
...
class AgentNA(mesa.Agent):
    def __init__(self, unique_id, model):
        ...
        self.type_id = 7
        self.goods_list = [("NAPottery", 1, 0.1)] * 10
        self.active = True
        self.agent_behavior = None
        self.trades_per_step = 0
        self.has_traded = False
...
class TestModelINTEST(mesa.Model):
    ...
    def meet_agents(self):
        """Function to facilitate meetings between agents on the same node."""
        # Iterate through each node in the graph
        for node in self.G.nodes:
            # Get all agents on the current node
            agents_on_node = [agent for agent in self.schedule.agents if agent.pos == node]
            print(f"Number of agents on node {node}: {len(agents_on_node)}")
            # Track agents that have already traded
            traded_agents = set()
            # If there is more than one agent on the node, they can meet
            if len(agents_on_node) > 1:
                # Randomly shuffle the list of agents
                random.shuffle(agents_on_node)
                # Select pairs of agents for meetings
                for i in range(0, len(agents_on_node) - 1, 2):
                    agent1 = agents_on_node[i]
                    agent2 = agents_on_node[i + 1]
                    # Only allow agents that haven't traded yet
                    if agent1 not in traded_agents and agent2 not in traded_agents:
                        if agent1.active and agent2.active and agent1.type_id != agent2.type_id:
                            self.trade(agent1, agent2)
                            print("Trade successful")
                            # Mark both agents as having traded
                            traded_agents.add(agent1)
                            traded_agents.add(agent2)
                        else:
                            print("Agent ended")
